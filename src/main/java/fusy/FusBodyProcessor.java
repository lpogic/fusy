...
public class FusBodyProcessor extends FusProcessor {
    ...
    public enum State {
        ...
        TRY_ID, TRY_SCOPE_EXP, AID_DOT, DOT_BRACE_STRING, DBS_END, TYPED_VAR, EXTENDS_AFTER_TYPE, EXTENDS_AFTER_EXP,
        ...
        if(!state.isEmpty()) {
            switch (state.peek()) {
                ...
                case DEFINITION,  FUSY_FUN, FUSY_TYPE, STATEMENT_VARIABLE, NEW_ARRAY,
                        INSTANCEOF, INLINE_DEFINITION -> subProcessor.advance(i);
                case STATEMENT -> {
                    switch (i) {
                        case '"' -> {
...
//                                result.append("fusy.this");
                ...
                case DOT -> {
                    switch (i) {
                        case '{' -> {
                            ...
                            state.push(State.DOT_BRACE_STRING);
                        }
                        default -> {
                            if (Character.isJavaIdentifierStart(i)) {
                                token = new StringBuilder();
                ...
                case DOT_BRACE_STRING -> {
                    switch (i) {
                        case '{' -> state.push(State.DOT_BRACE_STRING);
                        case '}' -> state.pop();
                        case '\\' -> result.append("\\\\");
